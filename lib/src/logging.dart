...
abstract class FfdLogs {
  static final widgetPainter = Logger("ffd.widget-painter");
  static final all = Logger("ffd");

  static final _activeLoggers = <Logger>{};
  
  /// Initialize the given [loggers] using the minimum [level].
  ///
  /// To enable all the loggers, use [FfdLogs.initAllLogs].
  ...
  ///
  /// To control which loggers are initialized, use [FfdLogs.initLoggers].
  static void initAllLogs([Level level = Level.ALL]) {
    initLoggers({all}, level);
  }
